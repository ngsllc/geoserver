...
public class FIPSKeyStoreProvider {
    ...
    public static void main(String[] args) {
        ...
        options.addOption(
                "c", "create-test-keystore", true, "Create a test PKCS12 keystore with a self-signed certificate");
        ...
        try {
            ...
            if (cmd.hasOption("create-test-keystore")) {
                String keystorePath = cmd.getOptionValue("create-test-keystore");
                String password = cmd.getOptionValue("password", "testpass");
                createTestKeystore(keystorePath, password);
                return;
            }
    ...
    /** Create a test JCEKS keystore with a secret key (no certificate required) */
    public static void createTestKeystore(String keystorePath, String password) {
        ...
        try {
            ...
            keyGen.init(128); // 128 bits for maximum compatibility
            javax.crypto.SecretKey secretKey = keyGen.generateKey();

            // Add secret key to keystore (no certificate required)
            keystore.setKeyEntry("test", secretKey, password.toCharArray(), null);
            ...
            System.out.println("✓ Test keystore created successfully");
            System.out.println("  Type: JCEKS");
            System.out.println("  Key algorithm: AES");
            System.out.println("  Key size: 128 bits");
            System.out.println("  Note: Contains secret key (no certificate required)");

        } catch (Exception e) {
            System.err.println("✗ Failed to create test keystore: " + e.getMessage());
            e.printStackTrace();
        }
    }
