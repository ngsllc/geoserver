...
/**
 * Builds the object that encrypts URL parameters, with a new AES-256 key for each session. See {@link AesCbcCrypt} for
 * the cipher.
 *
 * <p>It replaces the factory Wicket installs by default, which asks SunJCE for {@code PBEWithMD5AndDES}. That cipher
 * takes a password, but a FIPS-validated provider offers no password based cipher at all. Same design and cipher as
 * GeoServer upstream.
 */
public class KeyInSessionAesCryptFactory extends AbstractKeyInSessionCryptFactory<KeyInSessionAesCryptFactory.Key> {
    ...
    protected Key generateKey(Session session) {
        try {
            ...
            generator.init(256, FipsRuntime.secureRandom());
